...
export default function ImageUploadField({ name, bucket, defaultUrl, aspectHint }: Props) {
  ...
  async function handleFile(file: File) {
    ...
    start(async () => {
      try {
        ...
        if (pct > 0) {
          setSavings(`-${pct}% (${formatSize(compressed.originalSize)} → ${formatSize(compressed.compressedSize)})`);
        }
      } catch {
  ...
  return (
    <div>
      ...
      {savings && !pending && (
        <p style={{ fontSize: '0.56rem', color: '#6dbf7a', marginTop: '0.2rem', letterSpacing: '0.04em' }}>
          {savings}
        </p>
      )}
      <input
